...
During vLLM scale-up, connections are buffered for up to 2 minutes to prevent timeouts while the pod starts up and becomes ready.

### Activity Tracking
The proxy tracks request activity to determine when to scale down vLLM pods:
- Updates activity timestamp on each request
- Scales down after idle timeout (default 5 minutes)
- Maintains metrics for monitoring

### Error Handling
The proxy gracefully handles various failure scenarios:
- Model not found errors return available models list
- Scaling failures are logged and handled appropriately
- Streaming responses are properly managed for both OpenAI and Anthropic formats